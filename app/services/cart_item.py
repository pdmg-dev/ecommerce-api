from fastapi import HTTPException, status
from sqlalchemy.orm import Session

from app.models.cart_item import CartItem
from app.repositories.cart_item import CartItemRepository
from app.repositories.product import ProductRepository
from app.schemas.cart_item import CartItemCreate, CartItemUpdate
from app.utils import exceptions


class CartItemService:
    def __init__(self, db: Session):
        self.cart_item_repo = CartItemRepository(db)
        self.product_repo = ProductRepository(db)

    def list_user_cart(self, user_id: int):
        return self.cart_item_repo.get_user_cart_items(user_id)

    def add_to_cart(self, user_id: int, data: CartItemCreate):
        product = self.product_repo.get_by_id(data.product_id)
        if not product:
            raise exceptions.not_found("Product not found")

        existing = self.cart_item_repo.get_cart_item(user_id, data.product_id)
        if existing:
            # If already in cart, update quantity
            new_quantity = existing.quantity + data.quantity
            return self.product_repo.update_quantity(existing, new_quantity)

        new_item = CartItem(
            user_id=user_id,
            product_id=data.product_id,
            quantity=data.quantity,
        )
        return self.cart_item_repo.add_cart_item(new_item)

    def update_cart_item(self, user_id: int, product_id: int, data: CartItemUpdate):
        item = self.cart_item_repo.get_cart_item(user_id, product_id)
        if not item:
            raise exceptions.not_found("Cart item not found")
        return self.cart_item_repo.update_quantity(item, data.quantity)

    def remove_from_cart(self, user_id: int, product_id: int):
        item = self.cart_item_repo.get_cart_item(user_id, product_id)
        if not item:
            raise exceptions.not_found("Cart item not found")
        self.cart_item_repo.delete_item(item)
